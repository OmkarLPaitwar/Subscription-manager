...
exports.analyze = async (req, res) => {
  try {
    const subs = await Subscription.findAll({ where: { userId: req.user.id, status: 'Active' } });
    const now = new Date();
    const notifications = [];
    ...
    for (const sub of subs) {
      ...
      const wasUnused = sub.aiAnalysis?.isUnused || false;
      ...
      if (sub.aiAnalysis.isUnused && !wasUnused) {
        notifications.push({
          userId: req.user.id, subscriptionId: sub.id, type: 'unused_warning',
    ...
    for (const [cat, catSubs] of Object.entries(catMap)) {
      if (catSubs.length > 1) {
        ...
        notifications.push({
          userId: req.user.id, type: 'duplicate_detected',
